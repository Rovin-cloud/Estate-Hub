...
router.use(requireRole("client", "admin", "company_admin", "super_admin"));
...
router.get("/me", async (req, res): Promise<void> => {
  try {
    ...
    if (!customer) {
      res.status(404).json({ error: "No client profile found. Contact your sales representative." });
      return;
    }
...
router.get("/properties", async (req, res): Promise<void> => {
  try {
    ...
    const payments = await db.select().from(paymentsTable).where(eq(paymentsTable.customerId, customer.id));
